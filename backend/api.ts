...
api.get('/', (c) => ok(c, { status: 'ok', message: 'API is running' }));

api.post('/api/auth/check-username', async (c) => {
...
api.post('/api/auth/check-email', async (c) => {
...
api.post('/api/auth/update-email', async (c) => {
  const userId = parseAuth(c);
  if (!userId) return unauthorized(c);
...
api.post('/api/auth/update-phone', async (c) => {
  const userId = parseAuth(c);
  if (!userId) return unauthorized(c);
...
api.post('/api/verification/send-email-code', (c) => {
  const userId = parseAuth(c);
  if (!userId) return unauthorized(c);
  const code = genCode();
...
api.post('/api/verification/send-phone-code', (c) => {
  const userId = parseAuth(c);
  if (!userId) return unauthorized(c);
  const code = genCode();
...
api.post('/api/verification/verify-email', async (c) => {
  const userId = parseAuth(c);
  if (!userId) return unauthorized(c);
...
api.post('/api/verification/verify-phone', async (c) => {
  const userId = parseAuth(c);
  if (!userId) return unauthorized(c);
  const { code } = (await c.req.json().catch(() => ({}))) as { code?: string };
  if (!code || code.length !== 6) return badRequest(c, 'Invalid code');
...
api.post('/api/favorites/add', async (c) => {
  const userId = parseAuth(c);
  if (!userId) return unauthorized(c);
...
api.post('/api/favorites/remove', async (c) => {
  const userId = parseAuth(c);
  if (!userId) return unauthorized(c);
  const { attractionId } = (await c.req.json().catch(() => ({}))) as { attractionId?: string };
  if (!attractionId) return badRequest(c, 'Missing attractionId');
...
api.get('/api/favorites/list', (c) => {
  const userId = parseAuth(c);
  if (!userId) return unauthorized(c);
  return ok(c, Array.from(db.favorites.get(userId) ?? new Set()));
});

api.post('/api/visited/add', async (c) => {
  const userId = parseAuth(c);
  if (!userId) return unauthorized(c);
  const { attractionId } = (await c.req.json().catch(() => ({}))) as { attractionId?: string };
  if (!attractionId) return badRequest(c, 'Missing attractionId');
...
api.get('/api/visited/list', (c) => {
  const userId = parseAuth(c);
  if (!userId) return unauthorized(c);
  return ok(c, Array.from(db.visited.get(userId) ?? new Set()));
});

api.post('/api/reviews/add', async (c) => {
  const userId = parseAuth(c);
  if (!userId) return unauthorized(c);
...
api.post('/api/lists/rename', async (c) => {
  const userId = parseAuth(c);
  if (!userId) return unauthorized(c);
...
api.post('/api/lists/remove', async (c) => {
  const userId = parseAuth(c);
  if (!userId) return unauthorized(c);
...
api.post('/api/lists/add-item', async (c) => {
  const userId = parseAuth(c);
  if (!userId) return unauthorized(c);
...
api.post('/api/lists/remove-item', async (c) => {
  const userId = parseAuth(c);
  if (!userId) return unauthorized(c);
  const { listId, attractionId } = (await c.req.json().catch(() => ({}))) as { listId?: string; attractionId?: string };
  const li = db.lists.find((l) => l.user_id === userId && l.id === listId);
...
api.post('/api/checkins/create', async (c) => {
  const userId = parseAuth(c);
  if (!userId) return unauthorized(c);
...
api.post('/api/locations/search', async (c) => {
...
api.use('/api/trpc/*', trpcServer({ router: appRouter, createContext }));
